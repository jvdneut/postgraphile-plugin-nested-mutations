...
const graphQLInputObjectTypeFieldsHook = (fields, build, context) => {
	const {
		inflection,
...
const graphQLObjectTypeFieldsFieldHook = (field, build, context) => {
	...
	const recurseForwardNestedMutations = async (
		...
		await Promise.all(
			nestedFields.filter(k => input[k.name]).map(async nestedField => {
				const {
					constraint,
					foreignTable,
					keys,
...
module.exports = function PostGraphileNestedMutationPlugin(builder) {
	builder.hook(
		...
		graphQLInputObjectTypeFieldsHook,
	);

	builder.hook(
		'GraphQLObjectType:fields:field',
		graphQLObjectTypeFieldsFieldHook,
	);
};
